...
export const TrackedDebuggers = [
    ...
    'cspy', // IAR debugger
    'lldb', // LLDB debugger
    'lldb-dap', // LLDB debugger
    'cppvsdbg' // Microsoft debugger
];
...
export class DebuggerTrackerLocal {
    ...
    public static isValidSessionForMemory(id: string): string | boolean {
        ...
        if (!session) {
            return (
                'No session with the session id ' + id + '. Probably a bug or a debugger type that we are not tracking'
            );
        }
...
export class DebugTrackerFactory {
    ...
    public static register(cxt: vscode.ExtensionContext, debugTracker: DebugTracker): DebugTrackerFactory {
        DebugTrackerFactory.context = cxt;
        trackerApi = debugTracker;
    ...
    private settingsChanged(e: vscode.ConfigurationChangeEvent) {
        if (e.affectsConfiguration('memoryview.trackDebuggers')) {
    ...
    private initializeDebugTracker() {
        if (trackerApi) {
            ...
            if (typeof result === 'string') {
                vscode.window.showErrorMessage(`Subscription failed from debugTracker' : ${result}`);
            } else {
                trackerApiClientInfo = result;
            }
        } else {
            vscode.window.showErrorMessage('DebugTracker info is null.');
        }
    }
    ...
    static async debugTrackerEventHandler(event: IDebuggerTrackerEvent) {
        ...
        switch (event.event) {
            ...
            case OtherDebugEvents.Capabilities: {
                const good = DebuggerTrackerLocal.setCapabilities(
                    session,
                    event.capabilities as DebugProtocol.Capabilities
                );
                if (!good) {
                    tracker.deleteSelf();
...
function appendMsgToTmpDir(str: string) {
    try {
        ...
        if (false) {
            const fname = path.join(os.tmpdir(), 'memoryview-dbg-trace.txt');
