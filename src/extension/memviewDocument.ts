...
import {
    ...
    ICmdSetMemory,
    ICmdSetByte,
    IMemviewDocumentOptions,
    ITrackedDebugSessionXfer,
    ICmdClientState,
    ICmdGetStartAddress,
    ICmdButtonClick,
    ICmdSettingsChanged,
...
export class MemviewDocumentProvider implements vscode.CustomEditorProvider {
    private static readonly viewType = 'memoryview.memoryView';
    ...
    private readonly _onDidChangeCustomDocument = new vscode.EventEmitter<
        vscode.CustomDocumentContentChangeEvent<MemviewDocument>
    >();
    ...
    async openCustomDocument(
        ...
        if (!KNOWN_SCHEMES_ARRAY.includes(uri.scheme.toLocaleLowerCase())) {
            throw new Error(
                `Unsupported Uri scheme ${uri.scheme}. Allowed schemes are ${KNOWN_SCHEMES_ARRAY.join(', ')}`
            );
        }
    ...
    async resolveCustomEditor(
        ...
        webviewPanel.webview.html = MemviewDocumentProvider.getWebviewContent(
            ...
            this.context,
            JSON.stringify(memDoc.getOptions())
        );
    ...
    public static getWebviewContent(
        ...
        const scriptUri = webview.asWebviewUri(vscode.Uri.joinPath(context.extensionUri, 'dist', 'memview.js'));
        const styleUri = webview.asWebviewUri(vscode.Uri.joinPath(context.extensionUri, 'dist', 'memview.css'));
...
export class MemViewPanelProvider implements vscode.WebviewViewProvider, vscode.UriHandler {
    ...
    public handleUri(uri: vscode.Uri): vscode.ProviderResult<void> {
        try {
            ...
            if (existing.sessionInfo && existing.sessionInfo.status === DebugSessionStatus.Stopped) {
                MemViewPanelProvider.getExprResult(existing.sessionInfo.session, props.expr)
                    .then((addr) => {
                    ...
                    .catch((e) => {
                        ...
                        return Promise.reject(
                            new Error(`MemView URI handler: Expression ${expr} failed to evaluate: ${e}`)
                        );
                    });
            } else {
            ...
            case 'command': {
                ...
                switch (body.type) {
                    ...
                    case CmdType.ButtonClick: {
                        const doc =
                            body.docId && body.docId !== UnknownDocId
                                ? DualViewDoc.getDocumentById(body.docId)
                                : undefined;
                        ...
                        if (doc && newSettings) {
                            if (doc.expr !== newSettings.expr && doc.sessionStatus !== DocDebuggerStatus.Stopped) {
                                vscode.window.showInformationMessage(
                                    `Memory view expression changed to ${newSettings.expr}. ` +
    ...
    static addMemoryView(session: vscode.DebugSession, expr: string) {
        expr = expr.trim();
        MemViewPanelProvider.getExprResult(session, expr)
            .then((addr) => {
        ...
        return new Promise<string>((resolve, reject) => {
            ...
            session.customRequest('evaluate', arg).then((result) => {
                ...
                if (result.result) {
                    ...
                    vscode.window.showInformationMessage(
                        `Memory View: Address '${expr}' evaluated to ${res} which is not a constant`
                    );
                    reject(
                        new Error(`Expression '${expr}' failed to evaluate to a proper pointer value. Result: '${res}'`)
                    );
                } else {
                    vscode.window.showInformationMessage(`Memory View: Failed to evaluate address '${expr}'`);
                    reject(
                        new Error(`Expression '${expr}' failed to yield a proper result. Got ${JSON.stringify(result)}`)
                    );
                }
            }),
                (e: any) => {
