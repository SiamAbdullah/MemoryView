...
import { myGlobals, vscodePostCommandNoResponse } from './webviewGlobals';
...
export class DualViewDoc {
    ...
    setAddresses(startAddress: bigint) {
        ...
        // Originally we make sure the base address is aligned to 16 bytes by following calculation. (this.startAddress / 16n) * 16n;
        // But this is not needed anymore. We just use the start address as the base address like Visual Studio MemoryView.
    ...
    /*
        Memory address is send as hex value to debugger via DAP protocol.
        CppVSDBG does not support big int number as memory address for DAP protocol message.
    */
...
class MemPages {
    ...
    public getRowSync(addr: bigint): IByteVal[] {
        // Originally we make sure the base address is aligned to 16 bytes by following calculation. addr = (addr / 16n) * 16n;
        // commenting following code to align Visual Studio MemoryView panel.
        // addr = (addr / 16n) * 16n;
